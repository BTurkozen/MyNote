...
namespace MyNote.Desktop
{
    ...
    public partial class Form1 : Form
    {
        ...
        private string ApiUrl => "https://mynoteapi.burakturkozen.com/";
        private string UserName => txtUname.Text;
        private string Password => txtPass.Text;
        ...
        private async void btnListNote_Click(object sender, EventArgs e)
        {
                string token = await GetTokenAsync();

            if (token == null)
            {
                lstNote.DataSource = null;
                MessageBox.Show("Kullanıcı adı yada parola yanlış!");
                return;
            }
               // MessageBox.Show(token);
                List<Note> notes = await GetNotes(token);
                lstNote.DataSource = notes;
                lstNote.DisplayMember = "Title";
        ...
        private void lstNote_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var note = lstNote.SelectedItem as Note;
